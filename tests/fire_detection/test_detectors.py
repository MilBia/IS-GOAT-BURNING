...
def test_cpu_fire_detector() -> None:
...
def test_cuda_fire_detector() -> None:
...
def test_opencl_fire_detector() -> None:
...
def test_create_fire_detector_factory_returns_cpu_by_default() -> None:
...
@pytest.mark.skipif(cv2.cuda.getCudaEnabledDeviceCount() == 0, reason="No CUDA-enabled GPU found")
def test_create_fire_detector_factory_returns_cuda_when_requested() -> None:
...
@pytest.mark.skipif(not cv2.ocl.haveOpenCL(), reason="OpenCL is not available/enabled in this OpenCV build")
def test_create_fire_detector_factory_returns_opencl_when_requested() -> None:
